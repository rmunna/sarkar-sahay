...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const guide = await getMarathiGuideBySlug(slug);
  if (!guide) return {};
  ...
  return {
    ...
    openGraph: {
      title: guide.title,
      description: guide.description,
      url: `${BASE_URL}/mr/guide/${guide.slug}`,
      siteName: "CitizenNest",
      type: "article",
      locale: "mr_IN",
    },
    alternates: {
      canonical: `${BASE_URL}/mr/guide/${guide.slug}`,
      languages: {
        en: `${BASE_URL}/guide/${guide.slug}`,
        mr: `${BASE_URL}/mr/guide/${guide.slug}`,
      },
    },
  };
}
...
function extractHeadings(html: string): { id: string; text: string; level: number }[] {
  ...
  while ((match = regex.exec(html)) !== null) {
    ...
    const id = text.toLowerCase().replace(/[^a-z0-9ऀ-ॿ\s-]/g, "").replace(/\s+/g, "-").slice(0, 80);
...
function addHeadingIds(html: string): string {
  return html.replace(/<h([23])([^>]*)>(.*?)<\/h[23]>/gi, (match, level, attrs, text) => {
    ...
    const id = plainText.toLowerCase().replace(/[^a-z0-9ऀ-ॿ\s-]/g, "").replace(/\s+/g, "-").slice(0, 80);
...
export default async function MarathiGuidePage({ params }: Props) {
  const { slug } = await params;
  const guide = await getMarathiGuideBySlug(slug);
  if (!guide) notFound();

  const rawContent = getMarathiGuideRawContent(slug);
  ...
  const relatedGuides = getRelatedMarathiGuides(slug, 4);
  ...
  const breadcrumbSchema = {
    ...
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "मुखपृष्ठ", item: `${BASE_URL}/mr` },
      { "@type": "ListItem", position: 2, name: guide.category, item: `${BASE_URL}/mr` },
      { "@type": "ListItem", position: 3, name: guide.title, item: `${BASE_URL}/mr/guide/${guide.slug}` },
    ],
  };
  ...
  return (
    <div lang="mr">
      ...
      <nav className="text-sm text-gray-500 mb-6">
        <Link href="/mr" className="hover:text-orange-600 transition">मुखपृष्ठ</Link>
        <span className="mx-2">›</span>
        <Link href="/mr" className="hover:text-orange-600 transition">मार्गदर्शिका</Link>
        <span className="mx-2">›</span>
      ...
      <div className="lg:grid lg:grid-cols-[1fr_240px] lg:gap-10">
        <article className="bg-white rounded-2xl border border-gray-100 border-t-4 border-t-[#0f2744] shadow-sm px-6 py-8 sm:px-8 lg:px-10 lg:py-10">
          <header className="mb-8">
            ...
            {guide.readingTime && (
              <div className="flex items-center gap-1.5 text-sm text-gray-400">
                ...
                {guide.readingTime} वाचण्याची वेळ
              </div>
            )}
          </header>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 text-sm text-amber-800 flex gap-3">
            ...
            <div>
              <strong>टीप:</strong> हे एक स्वतंत्र माहिती मार्गदर्शक आहे. आम्ही कोणत्याही सरकारी संस्थेशी संबंधित नाही. नेहमी अधिकृत वेबसाइटवर पडताळा करा.
            </div>
          </div>
          ...
          {relatedGuides.length > 0 && (
            <div className="mt-12 pt-8 border-t border-gray-200">
              <h2 className="text-xl font-bold text-gray-900 mb-4">संबंधित मार्गदर्शिका</h2>
              <div className="grid gap-3 sm:grid-cols-2">
                {relatedGuides.map((rg) => (
                  <Link key={rg.slug} href={`/mr/guide/${rg.slug}`}
          ...
          <div className="mt-12 pt-8 border-t border-gray-200 flex flex-col sm:flex-row justify-between gap-4">
            <Link href="/mr" className="text-sm text-gray-500 hover:text-orange-600 transition">← सर्व मार्गदर्शिका</Link>
        ...
        {headings.length > 3 && (
          <aside className="hidden lg:block">
            <div className="sticky top-24 bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              <h3 className="text-xs font-bold text-[#0f2744] uppercase tracking-wider mb-3">या पानावर</h3>
