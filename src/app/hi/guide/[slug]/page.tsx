import { getHindiGuideBySlug, getHindiGuideRawContent, getRelatedHindiGuides, getAllHindiGuideSlugs } from "@/lib/guides-hi";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const guide = await getHindiGuideBySlug(slug);
  if (!guide) return {};
  ...
  return {
    ...
    openGraph: {
      title: guide.title,
      description: guide.description,
      url: `${BASE_URL}/hi/guide/${guide.slug}`,
      siteName: "CitizenNest",
      type: "article",
      locale: "hi_IN",
    },
    alternates: {
      canonical: `${BASE_URL}/hi/guide/${guide.slug}`,
      languages: {
        en: `${BASE_URL}/guide/${guide.slug}`,
        hi: `${BASE_URL}/hi/guide/${guide.slug}`,
      },
    },
  };
}
...
export default async function HindiGuidePage({ params }: Props) {
  const { slug } = await params;
  const guide = await getHindiGuideBySlug(slug);
  if (!guide) notFound();

  const rawContent = getHindiGuideRawContent(slug);
  ...
  const relatedGuides = getRelatedHindiGuides(slug, 5);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: guide.category, item: `${BASE_URL}/hi` },
      { "@type": "ListItem", position: 3, name: guide.title, item: `${BASE_URL}/hi/guide/${guide.slug}` },
    ],
  };
  ...
  return (
    <div lang="hi">
      ...
      <nav className="text-sm text-gray-500 mb-6">
        ...
        <Link href="/hi" className="hover:text-orange-600 transition">गाइड</Link>
        <span className="mx-2">›</span>
      ...
      <div className="lg:grid lg:grid-cols-[1fr_240px] lg:gap-10">
        <article>
            ...
            {guide.readingTime && (
              <div className="flex flex-wrap items-center gap-4 text-sm text-gray-400">
                <span className="flex items-center gap-1.5">
                  ...
                  {guide.readingTime} पढ़ने का समय
                </span>
              </div>
            )}
          </header>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-8 text-sm text-amber-800 flex gap-3">
            ...
            <div>
              <strong>अस्वीकरण:</strong> यह एक स्वतंत्र सूचनात्मक गाइड है।
              हम किसी भी सरकारी संस्था से संबद्ध नहीं हैं। कृपया हमेशा आधिकारिक वेबसाइट पर जाँच करें।
            </div>
          </div>
          ...
          {relatedGuides.length > 0 && (
            <div className="mt-12 pt-8 border-t border-gray-200">
              <h2 className="text-xl font-bold text-gray-900 mb-4">संबंधित गाइड</h2>
              <div className="grid gap-3 sm:grid-cols-2">
                {relatedGuides.map((rg) => (
                  <Link key={rg.slug} href={`/hi/guide/${rg.slug}`}
          ...
          <div className="mt-12 pt-8 border-t border-gray-200 flex flex-col sm:flex-row justify-between gap-4">
            <Link href="/hi" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-orange-600 transition">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
              </svg>
              सभी गाइड देखें
            </Link>
            <Link href="/" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-orange-600 transition">
              English Home
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {headings.length > 3 && (
          <aside className="hidden lg:block">
            <div className="sticky top-24">
              <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">इस पेज पर</h3>
