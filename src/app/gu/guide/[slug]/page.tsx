import { getGujaratiGuideBySlug, getGujaratiGuideRawContent, getRelatedGujaratiGuides, getAllGujaratiGuideSlugs } from "@/lib/guides-gu";
...
export async function generateStaticParams() {
  return getAllGujaratiGuideSlugs().map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const guide = await getGujaratiGuideBySlug(slug);
  if (!guide) return {};
  ...
  return {
    ...
    openGraph: {
      title: guide.title,
      description: guide.description,
      url: `${BASE_URL}/gu/guide/${guide.slug}`,
      siteName: "CitizenNest",
      type: "article",
      locale: "gu_IN",
    },
    alternates: {
      canonical: `${BASE_URL}/gu/guide/${guide.slug}`,
      languages: {
        en: `${BASE_URL}/guide/${guide.slug}`,
        gu: `${BASE_URL}/gu/guide/${guide.slug}`,
      },
    },
  };
}
...
function extractHeadings(html: string): { id: string; text: string; level: number }[] {
  ...
  while ((match = regex.exec(html)) !== null) {
    ...
    const id = text.toLowerCase().replace(/[^a-z0-9઀-૿\s-]/g, "").replace(/\s+/g, "-").slice(0, 80);
...
function addHeadingIds(html: string): string {
  return html.replace(/<h([23])([^>]*)>(.*?)<\/h[23]>/gi, (match, level, attrs, text) => {
    ...
    const id = plainText.toLowerCase().replace(/[^a-z0-9઀-૿\s-]/g, "").replace(/\s+/g, "-").slice(0, 80);
...
export default async function GujaratiGuidePage({ params }: Props) {
  const { slug } = await params;
  const guide = await getGujaratiGuideBySlug(slug);
  if (!guide) notFound();

  const rawContent = getGujaratiGuideRawContent(slug);
  ...
  const relatedGuides = getRelatedGujaratiGuides(slug, 4);
  ...
  const breadcrumbSchema = {
    ...
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "હોમ", item: `${BASE_URL}/gu` },
      { "@type": "ListItem", position: 2, name: guide.category, item: `${BASE_URL}/gu` },
      { "@type": "ListItem", position: 3, name: guide.title, item: `${BASE_URL}/gu/guide/${guide.slug}` },
    ],
  };
  ...
  return (
    <div lang="gu">
      ...
      <nav className="text-sm text-gray-500 mb-6">
        <Link href="/gu" className="hover:text-orange-600 transition">હોમ</Link>
        <span className="mx-2">›</span>
        <Link href="/gu" className="hover:text-orange-600 transition">માર્ગદર્શિકા</Link>
        <span className="mx-2">›</span>
      ...
      <div className="lg:grid lg:grid-cols-[1fr_240px] lg:gap-10">
        <article className="bg-white rounded-2xl border border-gray-100 border-t-4 border-t-[#0f2744] shadow-sm px-6 py-8 sm:px-8 lg:px-10 lg:py-10">
          <header className="mb-8">
            ...
            {guide.readingTime && (
              <div className="flex items-center gap-1.5 text-sm text-gray-400">
                ...
                {guide.readingTime} વાંચવાનો સમય
              </div>
            )}
          </header>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 text-sm text-amber-800 flex gap-3">
            ...
            <div>
              <strong>નોંધ:</strong> આ એક સ્વતંત્ર માહિતી માર્ગદર્શિકા છે. અમે કોઈ સરકારી સંસ્થા સાથે સંબંધિત નથી. હંમેશા સત્તાવાર વેબસાઇટ પર ચકાસો.
            </div>
          </div>
          ...
          {relatedGuides.length > 0 && (
            <div className="mt-12 pt-8 border-t border-gray-200">
              <h2 className="text-xl font-bold text-gray-900 mb-4">સંબંધિત માર્ગદર્શિકાઓ</h2>
              <div className="grid gap-3 sm:grid-cols-2">
                {relatedGuides.map((rg) => (
                  <Link key={rg.slug} href={`/gu/guide/${rg.slug}`}
          ...
          <div className="mt-12 pt-8 border-t border-gray-200 flex flex-col sm:flex-row justify-between gap-4">
            <Link href="/gu" className="text-sm text-gray-500 hover:text-orange-600 transition">← બધી માર્ગદર્શિકાઓ</Link>
        ...
        {headings.length > 3 && (
          <aside className="hidden lg:block">
            <div className="sticky top-24 bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              <h3 className="text-xs font-bold text-[#0f2744] uppercase tracking-wider mb-3">આ પાનામાં</h3>
