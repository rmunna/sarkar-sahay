...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // reciprocal hreflang only where a Hindi page actually exists (top schemes)
...
export default async function SchemePage({ params }: Props) {
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-8">
      ...
      <h1 className="text-2xl md:text-3xl font-bold text-gray-900 leading-tight">{s.name} Apply Online — Step-by-Step {s.level === "state" ? `${where} ` : ""}Guide</h1>
