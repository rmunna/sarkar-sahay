...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const guide = await getKannadaGuideBySlug(slug);
  if (!guide) return {};
  ...
  return {
    ...
    openGraph: {
      title: guide.title,
      description: guide.description,
      url: `${BASE_URL}/kn/guide/${guide.slug}`,
      siteName: "CitizenNest",
      type: "article",
      locale: "kn_IN",
    },
    alternates: {
      canonical: `${BASE_URL}/kn/guide/${guide.slug}`,
      languages: {
        en: `${BASE_URL}/guide/${guide.slug}`,
        kn: `${BASE_URL}/kn/guide/${guide.slug}`,
      },
    },
  };
}
...
function extractHeadings(html: string): { id: string; text: string; level: number }[] {
  ...
  while ((match = regex.exec(html)) !== null) {
    ...
    const id = text.toLowerCase().replace(/[^a-z0-9ಀ-೿\s-]/g, "").replace(/\s+/g, "-").slice(0, 80);
...
function addHeadingIds(html: string): string {
  return html.replace(/<h([23])([^>]*)>(.*?)<\/h[23]>/gi, (match, level, attrs, text) => {
    ...
    const id = plainText.toLowerCase().replace(/[^a-z0-9ಀ-೿\s-]/g, "").replace(/\s+/g, "-").slice(0, 80);
...
export default async function KannadaGuidePage({ params }: Props) {
  const { slug } = await params;
  const guide = await getKannadaGuideBySlug(slug);
  if (!guide) notFound();

  const rawContent = getKannadaGuideRawContent(slug);
  ...
  const relatedGuides = getRelatedKannadaGuides(slug, 4);
  ...
  const breadcrumbSchema = {
    ...
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "ಮುಖಪುಟ", item: `${BASE_URL}/kn` },
      { "@type": "ListItem", position: 2, name: guide.category, item: `${BASE_URL}/kn` },
      { "@type": "ListItem", position: 3, name: guide.title, item: `${BASE_URL}/kn/guide/${guide.slug}` },
    ],
  };
  ...
  return (
    <div lang="kn">
      ...
      {/* Language switcher — back to English */}
      ...
      <nav className="text-sm text-gray-500 mb-6">
        <Link href="/kn" className="hover:text-orange-600 transition">ಮುಖಪುಟ</Link>
        <span className="mx-2">›</span>
        <Link href="/kn" className="hover:text-orange-600 transition">ಮಾರ್ಗದರ್ಶಿ</Link>
        <span className="mx-2">›</span>
        ...
        <article className="bg-white rounded-2xl border border-gray-100 border-t-4 border-t-[#0f2744] shadow-sm px-6 py-8 sm:px-8 lg:px-10 lg:py-10">
          <header className="mb-8">
            ...
            {guide.readingTime && (
              <div className="flex items-center gap-1.5 text-sm text-gray-400">
                ...
                {guide.readingTime} ಓದುವ ಸಮಯ
              </div>
            )}
          </header>

          {/* Disclaimer in Kannada */}
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 text-sm text-amber-800 flex gap-3">
            <span className="text-lg leading-none mt-0.5">⚠️</span>
            <div>
              <strong>ಸೂಚನೆ:</strong> ಇದು ಒಂದು ಸ್ವತಂತ್ರ ಮಾಹಿತಿ ಮಾರ್ಗದರ್ಶಿ. ನಾವು ಯಾವುದೇ ಸರ್ಕಾರಿ ಸಂಸ್ಥೆಯೊಂದಿಗೆ ಸಂಬಂಧ ಹೊಂದಿಲ್ಲ. ಯಾವಾಗಲೂ ಅಧಿಕೃತ ವೆಬ್‌ಸೈಟ್‌ನಲ್ಲಿ ಪರಿಶೀಲಿಸಿ.
            </div>
          </div>
          ...
          {relatedGuides.length > 0 && (
            <div className="mt-12 pt-8 border-t border-gray-200">
              <h2 className="text-xl font-bold text-gray-900 mb-4">ಸಂಬಂಧಿತ ಮಾರ್ಗದರ್ಶಿಗಳು</h2>
              <div className="grid gap-3 sm:grid-cols-2">
                {relatedGuides.map((rg) => (
                  <Link key={rg.slug} href={`/kn/guide/${rg.slug}`}
          ...
          <div className="mt-12 pt-8 border-t border-gray-200 flex flex-col sm:flex-row justify-between gap-4">
            <Link href="/kn" className="text-sm text-gray-500 hover:text-orange-600 transition">
              ← ಎಲ್ಲಾ ಮಾರ್ಗದರ್ಶಿಗಳು
            </Link>
            <Link href={`/guide/${slug}`} className="text-sm text-gray-500 hover:text-orange-600 transition">
              English →
            </Link>
          </div>
        </article>
        ...
        {headings.length > 3 && (
          <aside className="hidden lg:block">
            <div className="sticky top-24 bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              <h3 className="text-xs font-bold text-[#0f2744] uppercase tracking-wider mb-3">ಈ ಪುಟದಲ್ಲಿ</h3>
