import { getTamilGuideBySlug, getTamilGuideRawContent, getRelatedTamilGuides, getAllTamilGuideSlugs } from "@/lib/guides-ta";
...
export async function generateStaticParams() {
  return getAllTamilGuideSlugs().map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const guide = await getTamilGuideBySlug(slug);
  if (!guide) return {};
  ...
  return {
    ...
    openGraph: {
      title: guide.title,
      description: guide.description,
      url: `${BASE_URL}/ta/guide/${guide.slug}`,
      siteName: "CitizenNest",
      type: "article",
      locale: "ta_IN",
    },
    alternates: {
      canonical: `${BASE_URL}/ta/guide/${guide.slug}`,
      languages: {
        en: `${BASE_URL}/guide/${guide.slug}`,
        ta: `${BASE_URL}/ta/guide/${guide.slug}`,
      },
    },
  };
}
...
export default async function TamilGuidePage({ params }: Props) {
  const { slug } = await params;
  const guide = await getTamilGuideBySlug(slug);
  if (!guide) notFound();

  const rawContent = getTamilGuideRawContent(slug);
  ...
  const relatedGuides = getRelatedTamilGuides(slug, 5);
  ...
  const breadcrumbSchema = {
    ...
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "முகப்பு", item: `${BASE_URL}/ta` },
      { "@type": "ListItem", position: 2, name: guide.category, item: `${BASE_URL}/ta` },
      { "@type": "ListItem", position: 3, name: guide.title, item: `${BASE_URL}/ta/guide/${guide.slug}` },
    ],
  };
  ...
  return (
    <div lang="ta">
      ...
      <nav className="text-sm text-gray-500 mb-6">
        <Link href="/ta" className="hover:text-orange-600 transition">முகப்பு</Link>
        <span className="mx-2">›</span>
        <Link href="/ta" className="hover:text-orange-600 transition">வழிகாட்டி</Link>
        <span className="mx-2">›</span>
      ...
      <div className="lg:grid lg:grid-cols-[1fr_240px] lg:gap-10">
        <article>
          <header className="mb-8">
            ...
            {guide.readingTime && (
              <div className="flex items-center gap-1.5 text-sm text-gray-400">
                ...
                {guide.readingTime} படிக்கும் நேரம்
              </div>
            )}
          </header>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-8 text-sm text-amber-800 flex gap-3">
            ...
            <div>
              <strong>அறிவிப்பு:</strong> இது ஒரு சுயாதீன தகவல் வழிகாட்டி. நாங்கள் எந்த அரசு நிறுவனத்துடனும் தொடர்புடையவர்கள் அல்ல. எப்போதும் அதிகாரப்பூர்வ இணையதளத்தில் சரிபார்க்கவும்.
            </div>
          </div>

          {guide.officialLinks.length > 0 && (
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 mb-8">
              <h2 className="text-sm font-bold text-blue-900 uppercase tracking-wide mb-3">அதிகாரப்பூர்வ இணைப்புகள்</h2>
          ...
          {relatedGuides.length > 0 && (
            <div className="mt-12 pt-8 border-t border-gray-200">
              <h2 className="text-xl font-bold text-gray-900 mb-4">தொடர்புடைய வழிகாட்டிகள்</h2>
              <div className="grid gap-3 sm:grid-cols-2">
                {relatedGuides.map((rg) => (
                  <Link key={rg.slug} href={`/ta/guide/${rg.slug}`}
          ...
          <div className="mt-12 pt-8 border-t border-gray-200 flex justify-between">
            <Link href="/ta" className="text-sm text-gray-500 hover:text-orange-600 transition">← அனைத்து வழிகாட்டிகளும்</Link>
        ...
        {headings.length > 3 && (
          <aside className="hidden lg:block">
            <div className="sticky top-24">
              <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">இந்த பக்கத்தில்</h3>
