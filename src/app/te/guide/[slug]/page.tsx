import { getTeluguGuideBySlug, getTeluguGuideRawContent, getRelatedTeluguGuides, getAllTeluguGuideSlugs } from "@/lib/guides-te";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const guide = await getTeluguGuideBySlug(slug);
  if (!guide) return {};
  ...
  return {
    ...
    openGraph: {
      title: guide.title,
      description: guide.description,
      url: `${BASE_URL}/te/guide/${guide.slug}`,
      siteName: "CitizenNest",
      type: "article",
      locale: "te_IN",
    },
    alternates: {
      canonical: `${BASE_URL}/te/guide/${guide.slug}`,
      languages: {
        en: `${BASE_URL}/guide/${guide.slug}`,
        te: `${BASE_URL}/te/guide/${guide.slug}`,
      },
    },
  };
}
...
function extractHeadings(html: string): { id: string; text: string; level: number }[] {
  ...
  while ((match = regex.exec(html)) !== null) {
    ...
    const id = text.toLowerCase().replace(/[^a-z0-9ఀ-౿\s-]/g, "").replace(/\s+/g, "-").slice(0, 80);
...
function addHeadingIds(html: string): string {
  return html.replace(/<h([23])([^>]*)>(.*?)<\/h[23]>/gi, (match, level, attrs, text) => {
    ...
    const id = plainText.toLowerCase().replace(/[^a-z0-9ఀ-౿\s-]/g, "").replace(/\s+/g, "-").slice(0, 80);
...
export default async function TeluguGuidePage({ params }: Props) {
  const { slug } = await params;
  const guide = await getTeluguGuideBySlug(slug);
  if (!guide) notFound();

  const rawContent = getTeluguGuideRawContent(slug);
  ...
  const relatedGuides = getRelatedTeluguGuides(slug, 5);
  ...
  const breadcrumbSchema = {
    ...
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "హోమ్", item: `${BASE_URL}/te` },
      { "@type": "ListItem", position: 2, name: guide.category, item: `${BASE_URL}/te` },
      { "@type": "ListItem", position: 3, name: guide.title, item: `${BASE_URL}/te/guide/${guide.slug}` },
    ],
  };
  ...
  return (
    <div lang="te">
      ...
      <nav className="text-sm text-gray-500 mb-6">
        <Link href="/te" className="hover:text-orange-600 transition">హోమ్</Link>
        <span className="mx-2">›</span>
        <Link href="/te" className="hover:text-orange-600 transition">గైడ్</Link>
        <span className="mx-2">›</span>
      ...
      <div className="lg:grid lg:grid-cols-[1fr_240px] lg:gap-10">
        <article>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-8 text-sm text-amber-800 flex gap-3">
            ...
            <div>
              <strong>నోటీసు:</strong> ఇది స్వతంత్ర సమాచార గైడ్. మేము ఏ ప్రభుత్వ సంస్థతోనూ అనుబంధించబడలేదు. దయచేసి ఎల్లప్పుడూ అధికారిక వెబ్‌సైట్‌లో తనిఖీ చేయండి.
            </div>
          </div>

          {guide.officialLinks.length > 0 && (
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 mb-8">
              <h2 className="text-sm font-bold text-blue-900 uppercase tracking-wide mb-3">అధికారిక లింకులు</h2>
          ...
          {relatedGuides.length > 0 && (
            <div className="mt-12 pt-8 border-t border-gray-200">
              <h2 className="text-xl font-bold text-gray-900 mb-4">సంబంధిత గైడులు</h2>
              <div className="grid gap-3 sm:grid-cols-2">
                {relatedGuides.map((rg) => (
                  <Link key={rg.slug} href={`/te/guide/${rg.slug}`}
          ...
          <div className="mt-12 pt-8 border-t border-gray-200 flex justify-between">
            <Link href="/te" className="text-sm text-gray-500 hover:text-orange-600 transition">← అన్ని గైడులు</Link>
        ...
        {headings.length > 3 && (
          <aside className="hidden lg:block">
            <div className="sticky top-24">
              <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">ఈ పేజీలో</h3>
