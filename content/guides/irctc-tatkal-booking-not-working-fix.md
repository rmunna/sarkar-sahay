---
title: "IRCTC Tatkal Ticket Booking Not Working — How to Fix"
description: "IRCTC tatkal booking failing? Fix payment errors, session expired, captcha issues, and server overload. Tips to book tatkal tickets faster."
category: "Utilities"
keywords:
  - irctc tatkal not working
  - tatkal ticket booking problem
  - irctc tatkal fix
  - irctc payment failed tatkal
  - irctc tatkal session expired
  - irctc tatkal server busy
  - tatkal booking tips
  - irctc tatkal payment deducted no ticket
  - irctc captcha error tatkal
...
officialLinks:
  ...
  - "https://contents.irctc.co.in/en/refundCancellationRules"
---

# IRCTC Tatkal Ticket Booking Not Working — How to Fix

Tatkal booking on IRCTC is one of the most stressful experiences for Indian train travellers. With lakhs of users trying to book at exactly 10:00 AM (AC classes) and 11:00 AM (sleeper class), the system faces extreme load — leading to payment failures, session timeouts, captcha errors, and "service unavailable" messages.
...
**How to fix:**
1. Do NOT refresh the page repeatedly — this puts you back in the queue
2. Wait 5–10 seconds and try again
3. Use the IRCTC website (www.irctc.co.in) — it handles load better than many third-party apps
...
2. Avoid SBI and other high-traffic bank gateways during tatkal hours — they are often the slowest
3. Keep your UPI app open and ready before starting the payment
4. If payment fails, check your bank account — if money was deducted, see the [Payment Deducted But No Ticket](#payment-deducted-but-ticket-not-booked) section below
...
**How to fix:**
1. **Pre-fill passenger details** — save all travellers as a master list in your IRCTC profile before tatkal time
2. Log in at least 5–10 minutes before tatkal opening (9:50 AM for AC, 10:50 AM for sleeper)
...
4. Consider using the IRCTC Rail Connect mobile app — it uses a simpler captcha system

### 5. "Tickets Are Not Available" — Train Sold Out Instantly

**Why it happens:** Tatkal quota is limited (around 30% of total seats in each class). High-demand routes sell out within seconds.

**What to do:**
1. This is not an error — the tickets genuinely sold out
2. Try [Premium Tatkal](#alternative-premium-tatkal-and-general-quota) (costs more but has separate quota)
...
|-------|-------------|-----------------|
| AC (1A, 2A, 3A, 3E, CC, EC) | 10:00 AM | 10:00–10:05 AM |
| Sleeper (SL) | 11:00 AM | 11:00–11:05 AM |

**Reality check:** On popular routes (Delhi–Mumbai, Delhi–Kolkata, Bangalore–Chennai), tatkal tickets can sell out in 30–60 seconds. The system is not broken — demand simply exceeds supply.

**What IRCTC does to manage load:**
- Virtual waiting room/queue system during peak time
- You may see "Request in queue" — do NOT close the browser
...
### Before Tatkal Time
1. **Save passenger master list:** Go to IRCTC → My Profile → Master List → add all passengers with full details (name, age, ID proof)
...
### During Booking
1. **Select passengers from master list** — don't type manually
2. **Skip travel insurance** — uncheck the insurance checkbox to save time
3. **Choose IRCTC iPay or UPI** for fastest payment processing
4. **Don't click "Pay" twice** — wait for the response even if it seems slow
...
1. Log in to IRCTC → My Transactions → Booked Ticket History
2. Look for the booking — sometimes the ticket IS booked but the confirmation page didn't load

### Step 2: Wait for Auto-Refund
- If the ticket was not booked, IRCTC initiates an **automatic refund within 5–7 working days**
- The amount is refunded to the same payment method used

### Step 3: File a TDR (if refund doesn't come)
1. Go to IRCTC → My Transactions → File TDR
2. Select the failed transaction
3. Submit with details of the payment

### Step 4: Contact IRCTC Support
- **IRCTC Care:** 14646 or 011-23340000
- **Email:** care@irctc.co.in
- **Online:** Lodge a complaint at [IRCTC Customer Care portal](https://www.irctc.co.in/)
- Keep your transaction ID, bank statement screenshot, and PNR (if generated) ready

For detailed refund tracking, see our [IRCTC Ticket Refund Not Received guide](/guide/irctc-ticket-refund-not-received-fix).
...
### Premium Tatkal
- **What:** Dynamic pricing tatkal — costs more but has a separate quota
...
### Other Options
- **RAC (Reservation Against Cancellation):** You get a shared berth — better than waitlist
...
1. IRCTC iPay (pre-loaded wallet) — fastest
2. UPI (Google Pay, PhonePe, Paytm) — very fast
3. Debit/Credit Card — moderate
4. Net Banking — slowest (avoid during tatkal)
...
Lakhs of users try to book tatkal simultaneously at 10:00 AM. IRCTC uses a queue system to handle the load. Wait a few seconds and try again — do not refresh repeatedly.

### Q2. My payment was deducted but I didn't get a tatkal ticket. Will I get a refund?
Yes. IRCTC auto-refunds failed transactions within 5–7 working days. If not received, file a TDR on the IRCTC portal or call 14646.

### Q3. Which is faster for tatkal booking — IRCTC website or app?
...
Regular tatkal has fixed extra charges. Premium tatkal has dynamic (higher) pricing but a separate quota — useful when regular tatkal sells out instantly.
...
> **Disclaimer:** CitizenNest is an independent informational platform and is not affiliated with IRCTC or Indian Railways. Information is based on publicly available official sources. Always verify details on [irctc.co.in](https://www.irctc.co.in/) before proceeding.
