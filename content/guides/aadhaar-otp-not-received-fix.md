---
title: "Aadhaar OTP Not Received 2026? Fix UIDAI Limit, DND Block & Wrong Mobile Number"
description: "Aadhaar OTP not coming? Fix: wrong mobile linked → visit Aadhaar Seva Kendra to update (₹50 fee); UIDAI OTP limit exceeded → wait 24 hours; DND block → use TOTP app instead. 8 working fixes including VID-based e-KYC alternative."
category: "Identity Documents"
keywords:
  - aadhaar otp not received
  - aadhaar otp not coming
  - aadhaar otp problem
  - aadhaar verification otp issue
  - uidai otp not received
  - aadhaar otp delay
  - aadhaar otp failed
  - aadhaar mobile otp fix
  - aadhaar ekyc otp not coming
  - myaadhaar otp issue
readingTime: 8 min
officialLinks:
...
# Aadhaar OTP Not Received: How to Fix
...
- **"OTP has been sent to your registered mobile number"** — but no SMS arrives
- **"OTP generation failed — please try again later"** — UIDAI server error
- **"Maximum OTP attempts exceeded"** — too many requests in a short period
- **"Invalid OTP"** — OTP received but does not work when entered
...
The most common reason. If your current mobile number is not linked to your Aadhaar, UIDAI sends the OTP to whatever number is on record — or no number at all.

**How to check:** Visit [myaadhaar.uidai.gov.in](https://myaadhaar.uidai.gov.in/) and try the "Verify Mobile/Email" option to see if your number is linked.
...
1. **Jio:** SMS `STOP` to 1909, or use MyJio app → Settings → DND
...
3. Each OTP is valid for 10 minutes — enter it promptly when received

### Step 6: Use TOTP Instead of OTP

UIDAI offers a TOTP (Time-based OTP) feature as an alternative:

1. Download the **mAadhaar app** from [Google Play](https://play.google.com/store/apps/details?id=in.gov.uidai.mAadhaar) or [App Store](https://apps.apple.com/in/app/maadhaar/id1435469474)
...
| **myAadhaar Portal** | [myaadhaar.uidai.gov.in](https://myaadhaar.uidai.gov.in/) |
| **UIDAI Grievance** | [uidai.gov.in/en/contact-support.html](https://uidai.gov.in/en/contact-support.html) |
...
1. **Keep your mobile number updated** — whenever you change your number, visit an Aadhaar centre to update it within 30 days
2. **Install mAadhaar app** — set up TOTP as a backup authentication method before you need it urgently
3. **Keep DND off** or ensure transactional messages are allowed through your DND preferences
4. **Don't spam OTP requests** — wait at least 2-3 minutes between attempts to avoid rate limiting
5. **Maintain SMS storage** — regularly clear old messages so new OTPs can be received
6. **Save UIDAI sender ID** — add the UIDAI SMS sender to your contacts so your phone does not filter it as spam
