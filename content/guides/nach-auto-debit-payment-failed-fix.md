---
title: "NACH/Auto-Debit Payment Failed — How to Fix EMI, SIP & Insurance Auto-Pay"
description: "NACH auto-debit payment failed? Fix EMI, SIP, insurance auto-pay failures. Check mandates, resolve bank rejections, and avoid penalties."
category: "Tax & Finance"
keywords:
  - nach payment failed
  - auto debit failed
  - emi auto debit not working
  - sip auto debit failed
  - nach mandate cancel
  - auto pay failed bank
  - nach mandate check
  - insurance auto debit failed
  - nach auto debit insufficient balance
...
- **Loan EMIs** — home loan, personal loan, car loan, education loan
- **SIP investments** — mutual fund Systematic Investment Plans
- **Insurance premiums** — life, health, and motor insurance
- **Subscription payments** — OTT platforms, utility bills, gym memberships
- **Government payments** — LPG subsidy credits, MNREGA wages (credit side)

When you sign up for any recurring payment, you authorize a **NACH mandate** — a standing instruction to your bank to debit a fixed or variable amount on specific dates.
...
If your account was migrated (e.g., during bank mergers like Dena Bank → Bank of Baroda, or Lakshmi Vilas Bank → DBS), the old mandate may no longer be valid.
...
2. Go to **e-Services → Manage BILLER** or **e-Services → NACH**
3. View all active mandates, their status, and limits
4. You can also check via **YONO SBI** app → Manage Mandates

### HDFC Bank
1. Log in to **NetBanking** (netbanking.hdfcbank.com)
2. Go to **Accounts → Manage e-Mandate / Auto-Pay**
3. View active, expired, and pending mandates
4. HDFC mobile app → **Pay → Manage AutoPay**

### ICICI Bank
1. Log in to **iMobile Pay** app or Internet Banking
2. Go to **Payments & Transfer → Manage Auto-Pay**
3. View all NACH/eNACH/UPI autopay mandates

### Axis Bank
1. Internet Banking → **Payments → Manage Auto-Debit**
2. Axis Mobile app → **More → Manage AutoPay**

### Kotak Mahindra Bank
1. Net Banking → **Bill Pay → Manage Auto Pay**
2. Kotak app → **AutoPay section**
...
A failed EMI auto-debit can result in **late payment charges** (typically ₹300-₹500 + GST per instance) and a negative impact on your **CIBIL/credit score**.

### Steps to Fix:

1. **Check your bank balance** — Ensure sufficient funds are available. Keep a buffer of ₹500-₹1,000 above the EMI amount.

2. **Make the payment manually** — Don't wait for the next auto-debit cycle. Pay immediately:
...
3. **Contact your lender** — Inform them about the failure. Ask them to:
   - Waive the late fee (often possible for first-time failures)
   - Re-initiate the auto-debit
   - Confirm the mandate is still active

4. **Set up balance alerts** — Enable SMS/email alerts for low balance (e.g., below ₹10,000) so you're warned before the debit date.

5. **Check mandate validity** — If your EMI tenure changed or interest rate reset caused a higher EMI, the old mandate limit may be insufficient. You'll need to **register a new mandate** with the updated amount.
...
- **2nd consecutive failure:** Another reminder; some AMCs charge a bounce fee (₹200-₹500)
...
### How to Fix:
1. **Pay the missed SIP manually** — Log in to your mutual fund platform (Groww, Zerodha Coin, Kuvera, MFCentral, or AMC website) and make a lump-sum purchase for the missed amount.

2. **Ensure balance on SIP date** — Most SIPs debit on the 1st, 5th, 10th, 15th, or 25th. Know your date and maintain balance.

3. **Check mandate status on MFCentral** — Visit mfcentral.com (SEBI-authorized) to view all your SIP mandates across AMCs.

4. **Re-register SIP if cancelled** — If your SIP was cancelled after 3 failures, you must:
   - Create a new SIP on your platform
   - Register a new NACH/eNACH mandate
   - Wait for mandate approval (2-7 working days)

5. **Switch to UPI AutoPay** — Many platforms now support UPI-based autopay (max ₹1 lakh per transaction). It's faster to set up and easier to manage.

## Insurance Premium Auto-Debit Failed — Policy Lapse Risk
...
- **If unpaid within grace period:** Policy lapses — no coverage for claims
...
### How to Fix:
1. **Pay immediately** — Use the insurer's app, website, or visit the branch. Most insurers accept NEFT, UPI, or cheque payments.

2. **Check if policy is in grace period** — Log in to the insurer's portal or call customer care. If within grace period, your coverage is still active.

3. **Set up a fresh mandate** — If the old mandate expired:
   - Contact the insurer to initiate a new eNACH mandate
   - Or set up via the insurer's app/portal

4. **Request policy revival** — If the policy has already lapsed:
...
1. Write to your bank with the mandate reference number (UMRN — Unique Mandate Reference Number)
2. If the bank doesn't act within 7 days, escalate to the Banking Ombudsman (rbi.org.in/Scripts/Complaints.aspx)

> **Warning:** Don't cancel a mandate for an active loan — the lender can levy penalties and report it as a default to CIBIL.
...
- For **recurring payments above ₹15,000**, Additional Factor Authentication (AFA) was previously required, but this has been relaxed for NACH-based mandates with proper registration
- You have the right to **dispute unauthorized debits** — file a complaint with your bank within 30 days

If you're not receiving pre-debit notifications, complain to your bank. They are legally required to send them.

## Important Tips

1. **Maintain a dedicated account for auto-debits** — Keep one salary or savings account specifically for EMIs and SIPs to avoid balance issues
2. **Set calendar reminders** — Mark your EMI/SIP dates and check balance 2 days before
3. **Keep ₹2,000-₹5,000 buffer** — Always maintain a buffer above your total auto-debit obligations
4. **Update mandates after rate resets** — When your loan EMI changes due to interest rate changes, update the mandate limit
5. **Review mandates quarterly** — Log in to your bank and review all active mandates; cancel ones you no longer need
...
Yes. You can have multiple active mandates on the same account — one for each service (e.g., home loan EMI, car loan EMI, SIP, insurance). There's no limit on the number of mandates per account.
