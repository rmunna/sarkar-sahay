...
function writeDeterministicCloudflarePage(source, change, registry, slugRegistry, announcement = null) {
  const stage = normalizeStage(change.stage || announcement?.type || 'notification');
  const examName = normalizeExamName(toTitleCase(
    announcement?.examName || examNameFromDetectorTitle(change.headline, source.name, change.pdfUrl || change.url)
  ));
  const key = dedupeKey(source.name, examName, stage);
  if (registry.has(key)) {
    return { generated: null, reason: `duplicate of ${registry.get(key).slug}`, examName };
  }

  const slug = generateSlug(source.name, examName, stage);
  if (slugRegistry.has(slug)) {
    return { generated: null, reason: `slug collision: ${slug}`, examName };
  }

  const noticeDate = normalizeDetectorDate(change.date);
  const officialUrl = change.pdfUrl || change.url;
  const stageLabel = stageLabelFor(stage);
  const title = `${examName} ${stageLabel} — Official SSC PDF`;
  const description = `${examName} ${stageLabel.toLowerCase()} is available on the official SSC website. Check the PDF notice date, official link and next steps before applying or checking results.`;
  const keywords = [
    `${examName.toLowerCase()} ${stageLabel.toLowerCase()}`,
    `${examName.toLowerCase()} official pdf`,
    `ssc ${stageLabel.toLowerCase()}`,
    `ssc.gov.in ${stageLabel.toLowerCase()}`
  ].filter(value => value.length <= 60);
  const fm = {
    title,
    description: description.length > 180 ? `${description.slice(0, 177)}...` : description,
    category: stage === 'result' ? 'Results' : 'Government Jobs',
    type: stage,
    organization: source.name,
    examName,
    stage,
    keywords: [...new Set(keywords)],
    importantDates: noticeDate ? { noticeDate } : {},
    officialLinks: [officialUrl, source.url].filter(Boolean).filter(isOfficialUrl),
    readingTime: '4 min',
  ...
  if (errors.length > 0) {
    return { generated: null, reason: `frontmatter validation failed: ${errors.join(', ')}`, examName };
  }

  const outPath = path.join(UPDATES_DIR, `${slug}.md`);
  const body = deterministicCloudflareBody({ source, change, fm, stageLabel, noticeDate, officialUrl });
  if (!DRY_RUN) {
    fs.writeFileSync(outPath, frontmatterToYaml(fm) + '\n\n' + body, 'utf8');
  ...
  return {
    ...
    generated: {
      ...
      confidence: Math.max(Number(change.confidence || 0), 0.82),
      fingerprints: [change.fingerprint].filter(Boolean)
    }
  };
}
...
function normalizeStage(stage) {
  ...
  if (['notification', 'admit-card', 'exam-schedule', 'result', 'cutoff', 'answer-key', 'registration'].includes(value)) {
    return value;
  }
  return 'notification';
}
...
function normalizeDetectorDate(value) {
...
function examNameFromDetectorTitle(title, sourceName, url = '') {
  let value = String(title || `${sourceName} Update`)
    .replace(/\s+/g, ' ')
    .replace(/\b(important\s+)?notice\s*[-:]\s*/ig, '')
    .replace(/\bnotice of\b/ig, '')
    .replace(/\bdeclaration of result\b/ig, 'Result')
    .trim()
    .slice(0, 110);
  if (/^(schedule of examination|notice|update)$/i.test(value)) {
    const fileName = detectorFileLabel(url);
    if (fileName) value = fileName;
  }
  return value;
}

function detectorFileLabel(url) {
  try {
    const pathname = new URL(url).pathname;
    const raw = decodeURIComponent(path.basename(pathname).replace(/\.pdf$/i, ''));
    return raw
      .replace(/^date\s+notice\s*/i, '')
      .replace(/^of\s+/i, '')
      .replace(/[_-]+/g, ' ')
      .replace(/\s+/g, ' ')
      .trim()
...
function deterministicCloudflareBody({ source, change, fm, stageLabel, noticeDate, officialUrl }) {
  const dateLine = noticeDate ? `The official notice date detected for this update is **${noticeDate}**.` : 'The official notice date was not available in a reliable machine-readable format.';
  const action = ({
    'Result': 'Open the official PDF and search for your roll number, registration number, list number, cut-off note, and instructions for the next stage.',
    'Exam Schedule': 'Open the official PDF and check the exam name, paper or post name, exam date, shift, reporting instructions, and any document requirements.',
    'Admit Card': 'Use the official SSC portal and the PDF instructions to confirm the admit card release process before entering credentials.',
    'Answer Key': 'Use the official PDF to confirm answer key availability, objection dates, fee instructions, and login requirements.',
    'Registration': 'Use the official PDF to confirm application start date, last date, fee, eligibility and the apply-online portal.',
    'Notification': 'Use the official PDF to confirm eligibility, vacancies, application dates, fee, syllabus and selection process.'
  })[stageLabel] || 'Open the official PDF and verify the exact details before taking action.';

  return `# ${fm.title}

SSC has published an official **${stageLabel.toLowerCase()}** PDF on [ssc.gov.in](${source.url}). CitizenNest detected this from the official SSC notice feed and is publishing this tracker without adding unverified eligibility, date or vacancy claims.
...
| Update | ${fm.examName} |
| Stage | ${stageLabel} |
| Official notice date | ${noticeDate || 'Check the PDF'} |
| Official PDF | [Download from SSC](${officialUrl}) |
| Official website | [ssc.gov.in](${source.url}) |

## What Is Confirmed

- The update is from the official SSC website.
- The official PDF link is available above.
- ${dateLine}
- No extra details on this page should be treated as a substitute for the official SSC PDF.

## What You Should Check In The PDF

${action}

For candidate-facing updates, check whether the PDF includes category-wise conditions, post-wise details, document verification instructions, region-wise links, login dates, and any corrigendum reference. If the PDF mentions a later detailed notice or regional website, follow that official link only.

## How To Open The Official SSC Notice

1. Open [ssc.gov.in](${source.url}).
2. Go to the notice or latest updates section.
3. Match the title with **${fm.examName}**.
4. Download the PDF and verify the exact details before applying, appearing for an exam, or checking result status.

## Safety Note

Do not use unofficial forms, Telegram links, or third-party payment links for SSC updates. For applications, admit cards, answer keys and results, use only the official SSC website or the official regional SSC websites linked from ssc.gov.in.

## FAQs

### Where can I download the official ${fm.examName} PDF?

Use the official PDF link on this page or visit [ssc.gov.in](${source.url}) and open the matching notice from SSC's notice section.

### Is CitizenNest the official SSC website?

No. CitizenNest is an independent guide site. Always verify the final details from [ssc.gov.in](${source.url}) before taking action.

### Why does this page not list all eligibility or vacancy details?

This page is a deterministic official-link tracker created from SSC's official feed. It avoids guessing details when automated PDF extraction or Gemini generation is unavailable. The official PDF is the source of truth.
`;
}
...
async function main() {
  ...
  for (const source of sourcesToProcess) {
    ...
    if (!searchResult?.found || !searchResult.announcement) {
      ...
      if (CLOUDFLARE_DETECTIONS && source._cloudflareChange?.pdfUrl) {
        const fallback = writeDeterministicCloudflarePage(source, source._cloudflareChange, registry, slugRegistry);
        if (fallback.generated) {
          log(`  ✅ Deterministic fallback written: ${fallback.path}`);
          results.generated.push(fallback.generated);
        } else {
          log(`  ⏭  Deterministic fallback skipped: ${fallback.reason}`);
          results.skipped.push({ source: source.name, reason: fallback.reason, exam: fallback.examName });
        }
      } else {
    ...
    if (!rawContentBody || rawContentBody.length < 200) {
      ...
      if (CLOUDFLARE_DETECTIONS && source._cloudflareChange?.pdfUrl) {
        const fallback = writeDeterministicCloudflarePage(source, source._cloudflareChange, registry, slugRegistry, ann);
        if (fallback.generated) {
          log(`  ✅ Deterministic fallback written: ${fallback.path}`);
          results.generated.push(fallback.generated);
        } else {
          log(`  ⏭  Deterministic fallback skipped: ${fallback.reason}`);
          results.skipped.push({ source: source.name, reason: fallback.reason, exam: fallback.examName || ann.examName });
        }
      } else {
